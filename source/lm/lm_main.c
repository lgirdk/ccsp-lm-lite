...
static PLmObjectHost XHosts_AddHostByPhysAddress (char *physAddress)
{
    ...
    if (pHost)
    {
        ...
        pHost->pStringParaValue[LM_HOST_Layer1InterfaceId] = AnscCloneString("Device.WiFi.Radio.1");
            ...
            (nname > 0))
    {
        *pList = AnscAllocateMemory(sizeof(Name_DM_t) * nname);
        ...
        if (*pList != NULL)
        {
			...
			/* Drop trailing dot, e.g. convert "Device.IP.Interface.4." -> "Device.IP.Interface.4", etc */
			len = strlen((*pList)[i].dm);
			if ((len > 0) && ((*pList)[i].dm[len - 1] == '.')) {
				(*pList)[i].dm[len - 1] = 0;
			}
...
void LM_get_host_state_behind_pod()
{
    ...
    {
        pHost = lmHosts.hostArray[i];
        if (pHost && (pHost->pStringParaValue[LM_HOST_AssociatedDeviceId] == NULL || AnscEqualString( pHost->pStringParaValue[LM_HOST_AssociatedDeviceId], " ", FALSE )) && AnscEqualString( pHost->pStringParaValue[LM_HOST_Layer1InterfaceId], "Unknown", FALSE ))
        {
            if(pHost->pStringParaValue[LM_HOST_IPAddressId])
            {
