...
char *schemaidbuffer = "d9823986-8092-4ee9-b1f6-cf808486f186/8e2a859d6ef44610423f559296565635";
...
long HostTableVersion = 0x0123456789012345;
int LineStatus = FALSE;

char* GetNDSchemaBuffer()
{
  return schemabuffer;
}

int GetNDSchemaBufferSize()
{
int len = 0;
if(schemabuffer)
	len = strlen(schemabuffer);
  
return len;
}

char* GetNDSchemaIDBuffer()
{
  return schemaidbuffer;
}

int GetNDSchemaIDBufferSize()
{
int len = 0;
if(schemaidbuffer)
        len = strlen(schemaidbuffer);

return len;
}

int NumberofElementsinLinkedList(struct networkdevicedata* head)
{
  int numelements = 0;
  struct networkdevicedata* ptr  = head;
  while (ptr != NULL)
  {
    numelements++;
    ptr = ptr->next;
...
avro_writer_t prepare_writer()
{
  ...
  if ( schema_file_parsed == FALSE )
  {
    ...
    /* open schema file */
    fp = fopen ( NETWORK_DEVICE_AVRO_FILENAME , "rb" );
    if ( !fp ) perror( NETWORK_DEVICE_AVRO_FILENAME " doesn't exist."), exit(1);
    ...
    schemabuffer = calloc( 1, lSize + 1 );

    if ( !schemabuffer ) fclose(fp), fputs("memory alloc fails", stderr), exit(1);

    /* copy the file into the buffer */
    if ( 1 != fread( schemabuffer , lSize, 1 , fp) )
      fclose(fp), free(schemabuffer), fputs("entire read fails", stderr), exit(1);

    fclose(fp);

    schema_file_parsed = TRUE; // parse schema file once only
    CcspLMLiteConsoleTrace(("RDK_LOG_DEBUG, Read Avro schema file ONCE, lSize = %ld, pbuffer = 0x%lx.\n", lSize + 1, (ulong)schemabuffer ));
  }
  else
  {
...
void lmlite_report_networkdevices(struct networkdevicedata *head)
{
  int i, j, k = 0;
  ...
  avro_schema_from_json(schemabuffer, strlen(schemabuffer),
